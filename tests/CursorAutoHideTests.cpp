...
void TestMouseActivityShowsCursor() {
	...
	Check(cursor.IsVisible(), "cursor should be visible after button/wheel activity");
	...
	// Consecutive activity while visible should refresh timestamp without re-triggering show callback
	cursor.OnButtonOrWheel(1500);
	Check(cursor.IsVisible(), "cursor should remain visible");
	Check(cursor.GetLastActivityMs() == 1500, "last activity timestamp should be updated");
	Check(show_count == 1, "show callback should not fire again when already visible");
}
...
void TestMotionFiltering() {
	...
	Check(cursor.GetLastActivityMs() == 1200, "activity timestamp updated on motion");
	...
	Check(cursor.GetLastActivityMs() == 1400, "activity timestamp updated on vertical motion");
}
...
void TestIdleAutoHiding() {
	...
	Check(cursor.IsVisible(), "cursor should stay visible before timeout expires");
	Check(hide_count == 0, "hide callback should not have fired yet");

	// Exactly at or after idle timeout
	cursor.CheckIdle(1000 + 2500);
	Check(!cursor.IsVisible(), "cursor should be auto-hidden when idle timeout reached");
...
void TestRemainingTimeoutCalculation() {
	...
	cursor.Show(1000);
	Check(cursor.GetRemainingTimeoutMs(1000) == 2500, "remaining timeout should be full delay immediately");
	Check(cursor.GetRemainingTimeoutMs(2000) == 1500, "remaining timeout should decrement accurately");
	Check(cursor.GetRemainingTimeoutMs(3500) == 0, "remaining timeout should be 0 when expired");
	Check(cursor.GetRemainingTimeoutMs(4000) == 0, "remaining timeout should clamp to 0 past expiry");

	cursor.Hide();
	Check(cursor.GetRemainingTimeoutMs(2000) == -1, "timeout must return -1 once hidden");
}
